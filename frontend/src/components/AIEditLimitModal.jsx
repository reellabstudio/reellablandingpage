...
export default function AIEditLimitModal({ open, onClose, message, locked = false, plan = "creator" }) {
  ...
  const buyAddon = async () => {
    ...
    try {
      ...
      alert(e.response?.data?.detail?.message || e.response?.data?.detail || "Couldn't open checkout");
      setBusy(false);
    }
  };

  const upgradeToCreator = () => { window.location.href = "/plan-checkout?plan=creator&billing=monthly"; };
  const upgradeToStudio = () => { window.location.href = "/plan-checkout?plan=studio&billing=monthly"; };

  // Free tier "feature locked" view — different from Creator's overage upsell
  const showLocked = locked || plan === "free";

  return (
    <div className="modal-overlay" onClick={onClose} data-testid="ai-edit-limit-modal">
      <div className="modal" onClick={(e) => e.stopPropagation()} style={{ maxWidth: 520, padding: 32, position: "relative" }}>
        ...
        <div style={{ display: "flex", justifyContent: "center", marginBottom: 18 }}>
          <div style={{ width: 56, height: 56, borderRadius: "50%", background: "var(--purple-glow)", display: "flex", alignItems: "center", justifyContent: "center", color: "var(--purple-mid)" }}>
            {showLocked ? <Lock size={24} /> : <Sparkles size={24} />}
          </div>
        </div>
        <h3 className="display-md" style={{ color: "var(--text)", textAlign: "center", margin: 0, marginBottom: 8 }}>
          {showLocked ? "AI Auto-Edit is locked on Free" : "AI edit limit reached"}
        </h3>
        <p style={{ color: "var(--text-sec)", textAlign: "center", fontSize: 13.5, marginBottom: 24, lineHeight: 1.5 }}>
          {message || (showLocked
            ? "AI Auto-Edit is a paid feature. Upgrade to Creator for 5/month, or grab Studio for unlimited."
            : "Top up below or upgrade to keep editing.")}
        </p>

        {showLocked ? (
          <>
            <button className="btn-primary" onClick={upgradeToCreator} style={{ width: "100%", padding: 14, marginBottom: 10 }} data-testid="ai-modal-upgrade-creator">
              Upgrade to Creator · $89/mo
            </button>
            <button onClick={upgradeToStudio} className="btn-secondary" style={{ width: "100%", padding: 12 }} data-testid="ai-modal-upgrade-studio">
              Or Studio for unlimited · $199/mo
            </button>
            <button onClick={onClose} className="btn-ghost" style={{ width: "100%", marginTop: 12 }} data-testid="ai-modal-decline">Not right now</button>
          </>
        ) : (
          <>
            ...
            <button className="btn-primary" onClick={buyAddon} disabled={busy} style={{ width: "100%", padding: 14 }} data-testid="ai-modal-buy">
